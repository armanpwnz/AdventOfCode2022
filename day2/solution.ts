...
export const solution = async () => {
  ...
  const file = await getFile(__dirname, 'input.txt')
...
const partOneScore = (rounds: Round[]): number => {
  const totalScore = rounds.reduce((prev: number, current: string) => {
    ...
    const opponentMove: Move = opponentMoveCode
      ? CODE_MOVE_MAP[opponentMoveCode as OpponentMoveCode]
      : 'Rock'
    const myMove: Move = myMoveCode
      ? CODE_MOVE_MAP[myMoveCode as MyMoveCode]
      : 'Rock'
    const score =
      RESULT_SCORE_MAP[ROUND_RESULT_MAP[`${opponentMove}|${myMove}`]] +
      MY_MOVE_BONUS_MAP[myMove]
    return prev + score
  }, 0)
  return totalScore
}
...
const partTwoScore = (rounds: Round[]): number => {
  const totalScore = rounds.reduce((prev: number, current: string) => {
    ...
    const desiredOutcome =
      CODE_ROUND_RESULT_MAP[desiredOutcomeCode as DesiredOutcomeCode]
